 ...
 */
export function lightTime (dist: number) {
 ...
 */
export function JulianYearToJDE (jy: number) {
 ...
 * @returns {Number} jde - Julian ephemeris day
 */
export function BesselianYearToJDE (by: number) {
 ...
 */
export function illuminated (i: number) {
 ...
 */
export function limb (equ: Coord, appSun: Coord) {
  const α = equ.ra
  const δ = equ.dec
  const α0 = appSun.ra
  const δ0 = appSun.dec
...
/** Return an indexed value or fail explicitly when a table is malformed. */
...
export function pmod (x: number, y: number) {
  let r = x % y
  if (r < 0) {
    r += y
  }
 ...
 * @returns {Number}
 */
export function horner (x: number, ...c: readonly number[]): number {
  if (c.length === 0) {
    throw new RangeError('horner requires at least one coefficient')
  }
 ...
 */
export function sincos (ε: number) : [number, number] {
 ...
 */
export function modf (float: number) : [number, number] {
 ...
 */
export function round (float: number, precision: number | undefined = 14) {
...
export function errorCode (msg: string, code: ErrorCode): CodeError {
  return new CodeError(msg, code)
}
