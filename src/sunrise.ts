...
import eqtime from './eqtime.ts'
import sexa from './sexagesimal.ts'
import solar from './solar.ts'
import julian, { Calendar } from './julian.ts'
...
const stdh0Sunrise = (refraction?: number) => rise.refraction(stdh0.sunrise, refraction)
const stdh0SunriseEnd = (refraction?: number) => rise.refraction(stdh0.sunriseEnd, refraction)
const stdh0Twilight = (refraction?: number) => rise.refraction(stdh0.twilight, refraction)
const stdh0NauticalTwilight = (refraction?: number) => rise.refraction(stdh0.nauticalTwilight, refraction)
const stdh0Night = (refraction?: number) => rise.refraction(stdh0.night, refraction)
const stdh0GoldenHour = (refraction?: number) => rise.refraction(stdh0.goldenHour, refraction)
...
export class Sunrise {
   ...
   * Methods may return `undefined` instead of `Calendar` for latitudes very near the poles.
   * @param {Calendar} date - calendar date
   ...
   */
  constructor (date: Calendar, lat: number, lon: number, refraction?: number) {
    this.date = date
  ...
  _calcRiseOrSet (jde: number, h0: number, isSet: boolean): number {
    const etime = sexa.secFromHourAngle(eqtime.eSmart(jde))
  ...
  _calcPolarDayNight (h0: number, isSet: boolean, step: number): Calendar | undefined {
    let jde = this.jde
    let t
    let failCnt = 0
    ...
    return new Calendar().fromJDE(jde + t)
  }

  _calc (h0: number, isSet: boolean): Calendar | undefined {
    let t
    const jde = this.jde
   ...
   * @return {Calendar} time of sunrise
   */
  rise () {
  ...
  /**
   * @return {Calendar} time of sunset
   ...
   * @return {Calendar} time of sunrise end
   */
  riseEnd () {
   ...
   * @return {Calendar} time of sunset start
   */
  setStart () {
   ...
   * @return {Calendar} time of nautical dawn
   */
  nauticalDawn () {
   ...
   * @return {Calendar} time of nautical dusk
   */
  nauticalDusk () {
   ...
   * @return {Calendar} time of start of night
   */
  nightStart () {
   ...
   * with astronomical observations.
   * @return {Calendar} time of end of night
   */
  nightEnd () {
   ...
   * @return {Calendar} time of start of golden hour
   */
  goldenHourStart () {
   ...
   * @return {Calendar} time of end of golden hour
   */
  goldenHourEnd () {
