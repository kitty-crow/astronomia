 ...
 */
export function astrometric (jde: number, earth: Planet) {
  const sε = base.SOblJ2000
  const cε = base.COblJ2000
  const f = function (jde: number) {
...
class Pt {
  ...
  constructor (i: number, j: number, k: number, lA: number, lB: number, bA: number, bB: number, rA: number, rB: number) {
