...
export class Len3 {
   ...
   */
  constructor (x1: number, x3: number, y: Array<number>) {
    if (y.length !== 3) {
      throw errorNot3
    }
    if (x3 === x1) {
    ...
    this.a = base.at(y, 1) - base.at(y, 0)
    this.b = base.at(y, 2) - base.at(y, 1)
  ...
  interpolateN (n: number) {
    return base.at(this.y, 1) + n * 0.5 * (this.abSum + n * this.c)
  }
  ...
  extremum () : [number, number] {
    ...
    const y = base.at(this.y, 1) - (this.abSum * this.abSum) / (8 * this.c) // (3.4), p. 25
    return [x, y]
  }
  ...
  zero (strong: boolean) {
    ...
    if (strong) {
      // (3.7), p. 27
      f = (n0: number) => {
        return n0 - (2 * base.at(this.y, 1) + n0 * (this.abSum + this.c * n0)) /
        (this.abSum + 2 * this.c * n0)
      }
    } else {
      // (3.6), p. 26
      f = (n0: number) => {
        return -2 * base.at(this.y, 1) / (this.abSum + this.c * n0)
      }
    }
 ...
 */
export function len3ForInterpolateX (x: number, x1: number, xN: number, y: Array<number>) {
  let y3 = y
  if (y.length > 3) {
 ...
 */
export const iterate = function (n0: number, f: (n: number) => number): [number, boolean] {
 ...
 */
export function len4Half (y: Array<number>) {
  if (y.length !== 4) {
    throw errorNot4
  }
  // (3.12) p. 32
  return (9 * (base.at(y, 1) + base.at(y, 2)) - base.at(y, 0) - base.at(y, 3)) / 16
}
...
export class Len5 {
   ...
   */
  constructor (x1: number, x5: number, y: readonly number[]) {
    if (y.length !== 5) {
      throw errorNot5
    }
    if (x5 === x1) {
    ...
    this.a = base.at(y, 1) - base.at(y, 0)
    this.b = base.at(y, 2) - base.at(y, 1)
    this.c = base.at(y, 3) - base.at(y, 2)
    this.d = base.at(y, 4) - base.at(y, 3)
  ...
  extremum () : [number, number] {
    ...
    const [n0, ok] = iterate(0, function (n0: number) {
 ...
 */
export function lagrange (x: number, table: ReadonlyArray<readonly [number, number]>) {
 ...
 */
export function lagrangePoly (table: ReadonlyArray<readonly [number, number] | { readonly x: number; readonly y: number }>) {
  // Method not fully described by Meeus, but needed for (numerical solution
  // to Example 3.g.
  const sum = new Array<number>(table.length).fill(0)
  const prod = new Array<number>(table.length).fill(0)
  const last = table.length - 1
  for (let i = 0; i < table.length; i++) {
    const row = table[i]
    if (!row) continue
    const xi = 'x' in row ? row.x : row[0]
    const yi = 'y' in row ? row.y : row[1]
    prod[last] = 1
    let den = 1.0
    let n = last
    for (let j = 0; j < table.length; j++) {
      if (i !== j) {
        const other = table[j]
        if (!other) continue
        const xj = 'x' in other ? other.x : other[0]
        prod[n - 1] = base.at(prod, n) * -xj
        for (let k = n; k < last; k++) {
          prod[k] = base.at(prod, k) - base.at(prod, k + 1) * xj
        }
        n--
        den *= (xi - xj)
      }
    }
    prod.forEach((pj: number, j: number) => {
      sum[j] = base.at(sum, j) + yi * pj / den
    })
  }
  return sum
}
 ...
 */
export function linear (x: number, x1: number, xN: number, y: readonly number[]) {
  const interval = (xN - x1) / (y.length - 1)
  if (interval === 0) {
    throw errorNoXRange
  }
  ...
  return base.at(y2, 0) + (base.at(y, 1) - base.at(y, 0)) * (x - x01) / interval
}
