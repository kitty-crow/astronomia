...
export type VSOPType = 'B' | 'D'
export type VSOPTerm = readonly [a: number, b: number, c: number]
export type VSOPSeries = Readonly<Record<string, readonly VSOPTerm[]>>

export interface VSOP87Planet {
  name?: string
  type?: VSOPType
  L: VSOPSeries
  B: VSOPSeries
...
function sum (t: number, series: VSOPSeries): number {
  const coeffs = Object.keys(series)
    ...
    .map((key) => {
      ...
      for (let i = terms.length - 1; i >= 0; i--) {
        const [a, b, c] = base.at(terms, i)
        value += a * Math.cos(b + c * t)
      }
      return value
    })
...
export class Planet {
  ...
  readonly type: VSOPType
  private readonly series: VSOP87Planet
 ...
 */
export function toFK5 (lon: number, lat: number, jde: number) {
  // formula 32.3, p. 219.
  const T = base.J2000Century(jde)
