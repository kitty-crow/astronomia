 ...
 */
export function horizontal (Δ: number) {
  // (40.1) p. 279
 ...
 */
export function topocentric (c: Coord, ρsφ: number, ρcφ: number, lon: number, jde: number) {
 ...
 * @return {Coord} observed topocentric ra and dec in radians.
 */
export function topocentric2 (c: Coord, ρsφ: number, ρcφ: number, lon: number, jde: number) {
  const [α, δ, Δ] = [c.ra, c.dec, c.range]
  const π = horizontal(Δ)
  const θ0 = new sexa.Time(sidereal.apparent(jde)).rad()
  const H = base.pmod(θ0 - lon - α, 2 * Math.PI)
 ...
 */
export function topocentric3 (c: Coord, ρsφ: number, ρcφ: number, lon: number, jde: number) : [number, number] {
  const [α, δ, Δ] = [c.ra, c.dec, c.range]
  const π = horizontal(Δ)
  const θ0 = new sexa.Time(sidereal.apparent(jde)).rad()
  const H = base.pmod(θ0 - lon - α, 2 * Math.PI)
 ...
 */
export function topocentricEcliptical (c: Coord, s: number, φ: number, h: number, ε: number, θ: number, π: number) : [number, number, number] {
