...
export class Calendar {
   ...
   * @param {number} [day]
   */
  constructor (year: number | Date = 0, month = 1, day = 1) {
  ...
  fromYear (year: number) {
    ...
    let m = 12
    while (m > 0 && base.at(DAYS_OF_YEAR, m) > dayOfYear) {
      m--
    }
    this.month = m
    this.day = 1 + dayOfYear - base.at(DAYS_OF_YEAR, this.month)
    return this
  }
 ...
 */
export function LeapYearJulian (y: number) {
  return y % 4 === 0
}
 ...
 */
export function LeapYearGregorian (y: number) {
 ...
 * @returns {boolean} true for Gregorian, false for Julian calendar
 */
export function isCalendarGregorian (year: number, month = 1, day = 1): boolean {
  return (year > 1582 ||
    (year === 1582 && month > 10) ||
...
export function DayOfYear (y: number, m: number, d: number, leap: boolean) {
  ...
  return k + base.at(DAYS_OF_YEAR, m) + int(d)
}
...
export function DayOfYearToCalendar (n: number, leap: boolean) {
  ...
  for (month = 1; month <= 12; month++) {
    if (k + base.at(DAYS_OF_YEAR, month) > n) {
      month = month - 1
      break
    }
  }
  const day = n - k - base.at(DAYS_OF_YEAR, month)
  return { month, day }
}
