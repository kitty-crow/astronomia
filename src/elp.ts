...
import base, { Coord } from './base.ts'

export type ElpTerm = readonly [amplitude: number, ...phase: number[]]
export type ElpSeries = Readonly<Record<string, readonly ElpTerm[]>>

export interface ElpData {
  readonly name?: string
  readonly W1: readonly number[]
  readonly L: ElpSeries
  readonly B: ElpSeries
  readonly R: ElpSeries
}
...
function sum (T: number, series: ElpSeries): number {
  const coeffs: number[] = []
  for (const [key, rows] of Object.entries(series)) {
    ...
    for (let y = rows.length - 1; y >= 0; y--) {
      const row = rows[y]!
      const [amplitude, ...phase] = row
      coefficient += amplitude * Math.sin(base.horner(T, ...phase))
    }
    coeffs[order] = coefficient
  }
...
export class Moon {
   ...
   *   {Number} z
   */
  positionXYZ (jde: number): RectangularCoordinates {
 ...
 */
export function position (elpData: ElpData, jde: number): Coord {
