 ...
 */
export function time (r1: number, d1: number, r2: number, d2: number, r3: Array<number>, d3: Array<number>, t1: number, t5: number) { // (r1, d1, r2, d2 float64, r3, d3 []float64, t1, t5 float64)  (float64, error)
  if (r3.length !== 5 || d3.length !== 5) {
    throw new Error('r3, d3 must be length 5')
  }
  const gc: number[] = new Array(5)
  r3.forEach((r3i: number, i: number) => {
    // (19.1) p. 121
    gc[i] = Math.tan(d1) * Math.sin(r2 - r3i) +
      Math.tan(d2) * Math.sin(r3i - r1) +
      Math.tan(base.at(d3, i)) * Math.sin(r1 - r2)
  })
 ...
 * Algorithm by Meeus.
 */
export function angle (r1: number, d1: number, r2: number, d2: number, r3: number, d3: number) { // (r1, d1, r2, d2, r3, d3 float64)  float64
 ...
 * Algorithm by Meeus.
 */
export function error (r1: number, d1: number, r2: number, d2: number, r0: number, d0: number) { // (r1, d1, r2, d2, r0, d0 float64)  float64
 ...
 */
export function angleError (r1: number, d1: number, r2: number, d2: number, r3: number, d3: number) : [number, number] {
  const [sr1, cr1] = base.sincos(r1)
