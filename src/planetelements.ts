...
export class Elements {
  ...
  constructor (
    ...
    const o: ElementsInit = typeof lon === 'object' ? lon : {}
    this.lon = o.lon ?? (typeof lon === 'number' ? lon : 0)
 ...
 * Table 31.A, p. 212
 */
const cMean: Record<PlanetName, MeanCoeff> = {
  mercury: { // Mercury
 ...
 */
export function mean (p: PlanetName, jde: number, e = new Elements()): Elements {
  ...
  e.node = c.Ω ? base.horner(T, ...c.Ω) * Math.PI / 180 : 0
 ...
 */
export function inc (p: PlanetName, jde: number) { // (p int, jde float64)  float64
 ...
 * radians, referenced to mean dynamical ecliptic and equinox of date.
 */
export function node (p: PlanetName, jde: number) { // (p int, jde float64)  float64
  const Ω = cMean[p].Ω
  return Ω ? base.horner(base.J2000Century(jde), ...Ω) * Math.PI / 180 : 0
}
