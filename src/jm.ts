 ...
 */
export function JewishCalendar (y: number) : [number, number, number, number, number, number, number] {
  const A = y + 3760
  const D = bigD(y)
  let mP = 3
  let dP = D
...
const bigD = function (y: number) { // (y int)  int
 ...
 */
export function MoslemToJD (y: number, m: number, d: number) { // (y, m, d int)  (jY, jDN int)
 ...
 */
export function MoslemLeapYear (year: number) { // (y int)  bool
  const R = year % 30
  return (11 * R + 3) % 30 > 18
}
 ...
 * @returns {DateObj}
 */
export function JulianToMoslem (y: number, m: number, d: number) { // (y, m, d int)  (my, mm, md int)
  let W = 2
  if (y % 4 === 0) {
    W = 1
  }
 ...
 */
export function moslemMonth (m: number) {
  return mmonths[m]
}
