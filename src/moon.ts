...
import moonposition from './moonposition.ts'
 ...
 */
export function physical (jde: number, earth: Planet) : [Coord, number, Coord] {
...
export class Moon {
  ...
  optical (λ: number, β: number) : [number, number, number] {
    // (53.1) p. 372
  ...
  physical (A: number, b_: number) : [number, number] {
    // (53.2) p. 373
  ...
  sun (λ: number, β: number, Δ: number, earth: Planet): [number, number] {
 ...
 */
export function sunAltitude (cOnMoon: Coord, cSun: Coord) { // (η, θ, l0, b0 float64)  float64
 ...
 */
export function sunrise (cOnMoon: Coord, jde: number, earth: Planet) { // (η, θ, jde float64, earth *pp.V87Planet)  float64
 ...
 */
export function sunset (cOnMoon: Coord, jde: number, earth: Planet) { // (η, θ, jde float64, earth *pp.V87Planet)  float64
  jde += srCorr(cOnMoon, jde, earth)
  return jde + srCorr(cOnMoon, jde, earth)
}
 ...
 * @private
 */
function srCorr (cOnMoon: Coord, jde: number, earth: Planet): number {
  const phy = physical(jde, earth)
  const h = sunAltitude(cOnMoon, phy[2])
  return h / (12.19075 * D2R * cos(cOnMoon.lat))
}

const lunarCoord = (η: number, θ: number) => new base.Coord(η * D2R, θ * D2R)
/**
