...
type LongitudeRadiusRow = readonly [number, number, number, number, number, number]
type LatitudeRow = readonly [number, number, number, number, number]
 ...
 */
export function parallax (distance: number) {
  // p. 337
  return asin(EARTH_RADIUS / distance)
}

function dmf (T: number) : [number, number, number, number] {
...
const ta = (function () {
  const ta: readonly LongitudeRadiusRow[] = [
  ...
  return ta.map((row): LongitudeRadiusTerm => {
    const [d, m, m_, f, Σl, Σr] = row
    return { d, m, m_, f, Σl, Σr }
  })
})()

const tb = (function () {
  const tb: readonly LatitudeRow[] = [
    [0, 0, 0, 1, 5128122],
    [0, 0, 1, 1, 280602],
    [0, 0, 1, -1, 277693],
    [2, 0, 0, -1, 173237],
  ...
  return tb.map((row): LatitudeTerm => {
    const [d, m, m_, f, Σb] = row
    return { d, m, m_, f, Σb }
  })
})()
