 ...
 */
export function oneDegreeOfLongitude (rp: number) {
 ...
 * @return {number} distance in Km
 */
export function oneDegreeOfLatitude (rm: number) {
 ...
 */
export function geocentricLatitudeDifference (φ: number) {
 ...
 */
export function approxAngularDistance (p1: Coord, p2: Coord) {
 ...
 */
export function approxLinearDistance (d: number) {
  return 6371 * d
}
 ...
 * @private
 */
function sincos2 (x: number) : [number, number] {
  const s = Math.sin(x)
  const c = Math.cos(x)
  return [s * s, c * c]
}
