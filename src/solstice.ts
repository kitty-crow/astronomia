...
import type { Planet } from './planetposition.ts'
...
// table 27.c
interface PeriodicTerm { readonly a: number; readonly b: number; readonly c: number }
type PeriodicRow = readonly [number, number, number]

const terms = (function () {
  const term: readonly PeriodicRow[] = [
    [485, 324.96, 1934.136],
  ...
  return term.map((t): PeriodicTerm => {
    return {
      a: t[0],
      b: t[1],
      c: t[2]
 ...
 * @returns {Number} JDE
 */
export function march (y: number) {
  if (y < 1000) {
    return eq(y, mc0)
  }
 ...
 * @returns {Number} JDE
 */
export function september (y: number) {
  if (y < 1000) {
    return eq(y, sc0)
  }
 ...
 * @returns {Number} JDE
 */
export function december (y: number) {
  if (y < 1000) {
    return eq(y, dc0)
  }
 ...
 * @returns {Number} JDE
 */
function eq (y: number, c: readonly number[]) {
 ...
 * @returns {Number} JDE
 */
export function march2 (year: number, planet: Planet) {
 ...
 * @returns {Number} JDE
 */
export function june2 (year: number, planet: Planet) {
 ...
 * @returns {Number} JDE
 */
export function september2 (year: number, planet: Planet) {
 ...
 * @returns {Number} JDE
 */
export function december2 (year: number, planet: Planet) {
 ...
 * @returns {Number} JDE
 */
export function longitude (year: number, planet: Planet, lon: number) {
  let c
  let ct
  ...
  return eq2(year, planet, lon, c ?? (() => { throw new RangeError('missing solstice coefficients') })())
}
...
/**
  ...
  * @returns {Number} JDE
  */
function eq2 (year: number, planet: Planet, lon: number, c: readonly number[]) {
