...
import planetposition, { Planet } from './planetposition.ts' // eslint-disable-line no-unused-vars
 ...
 */
export function positions (jde: number, earth: Planet, saturn: Planet) {
  ...
  const Y = new Array<number>(9).fill(0)
  const Z = new Array<number>(9).fill(0)
  for (let j = 1; j <= 8; j++) {
    const moon = base.at(s4, j)
    const u = moon.λ - moon.Ω
    const w = moon.Ω - 168.8112 * d
    const [su, cu] = base.sincos(u)
    const [sw, cw] = base.sincos(w)
    const [sγ, cγ] = base.sincos(moon.γ)
    const r = moon.r
  ...
  for (let j = 0; j < X.length; j++) {
    let a = base.at(X, j)
    let b = q.c1 * base.at(Y, j) - q.s1 * base.at(Z, j)
    const c = q.s1 * base.at(Y, j) + q.c1 * base.at(Z, j)
  ...
  const pos = new Array<XYZ>(8)
  const D = Math.atan2(base.at(A, 0), base.at(C, 0))
  const [sD, cD] = base.sincos(D)
  for (let j = 1; j <= 8; j++) {
    const moon = base.at(s4, j)
    X[j] = base.at(A, j) * cD - base.at(C, j) * sD
    Y[j] = base.at(A, j) * sD + base.at(C, j) * cD
    Z[j] = base.at(B, j)
    const d = base.at(X, j) / moon.r
    X[j] = base.at(X, j) + Math.abs(base.at(Z, j)) / base.at(k, j) * Math.sqrt(1 - d * d)
    const W = Δ / (Δ + base.at(Z, j) / 2475)
    pos[j - 1] = new XYZ(base.at(X, j) * W, base.at(Y, j) * W, base.at(Z, j))
  }
  return pos
}
...
class R4 {
  ...
  constructor (λ?: number, r?: number, γ?: number, Ω?: number) {
...
export class Qs {
  ...
  subr (λʹ: number, p: number, e: number, a: number, Ω: number, i: number) {
  const r = new R4()
  const M = λʹ - p
  const e2 = e * e
  const e3 = e2 * e
