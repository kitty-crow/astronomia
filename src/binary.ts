 ...
 */
export function meanAnomaly (year: number, T: number, P: number) { // (year, T, P float64)  float64
  const n = 2 * Math.PI / P
 ...
 */
export function position (a: number, e: number, i: number, Ω: number, ω: number, E: number) : [number, number] { // (a, e, i, Ω, ω, E float64)  (θ, ρ float64)
  const r = a * (1 - e * cos(E))
 ...
 */
export function apparentEccentricity (e: number, i: number, ω: number) { // (e, i, ω float64)  float64
  const cosi = cos(i)
