...
type NutationRow = readonly [number, number, number, number, number, number, number, number, number]
 ...
 * @return {number[]} [Δψ, Δε] - [longitude, obliquity] in radians
 */
export function approxNutation (jde: number) : [number, number] {
...
const table22A = (function () {
  const tab: readonly NutationRow[] = [
  ...
  return tab.map((row): NutationTerm => {
    const [d, m, n, f, ω, s0, s1, c0, c1] = row
    return { d, m, n, f, ω, s0, s1, c0, c1 }
  })
})()
