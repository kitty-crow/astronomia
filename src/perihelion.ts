...
export type PlanetCode = typeof planetsEnum[keyof typeof planetsEnum]
type RoundFunction = (x: number) => number
export type Extremum = [jde: number, range: number]
export type ExtremumCallback = (result: Extremum) => void
 ...
 */
export function perihelion (p: PlanetCode, year: number) {
 ...
 * @returns {Number} jde - time of the event
 */
export function aphelion (p: PlanetCode, year: number) {
  return ap(p, year, true, af)
}

const pf = function (x: number) { // (x float64)  float64
  return Math.floor(x + 0.5)
}

const af = function (x: number) { // (x float64)  float64
  return Math.floor(x) + 0.5
}

const ap = function (p: PlanetCode, y: number, a: boolean, f: RoundFunction): number { // (p int, y float64, a bool, f func(float64)  float64) float64
  let i = p
  if (i === embary) {
    i = earth
  }
  const constants = base.at(ka, i)
  const k = f(constants.a * (y - constants.b))
  let j = base.horner(k, ...base.at(c, i))
  if (p === earth) {
    let c = ep
    if (a) {
      c = ea
    ...
    for (let i = 0; i < 5; i++) {
      const correction = base.at(c, i)
      const angle = base.at(ec, i)
      j += correction * Math.sin((angle.a + angle.b * k) * Math.PI / 180)
    }
  }
  return j
}
 ...
 */
export function perihelion2 (planet: Planet, year: number, precision: number): Extremum
export function perihelion2 (planet: Planet, year: number, precision: number, cb: ExtremumCallback): void
export function perihelion2 (planet: Planet, year: number, precision: number, cb?: ExtremumCallback): Extremum | void {
  const code = planetCode(planet.name)
  return cb ? ap2(code, year, precision, planet, false, pf, cb) : ap2(code, year, precision, planet, false, pf)
}
 ...
 *   {Number} r - the distance of the planet from the Sun in AU.
 */
export function aphelion2 (planet: Planet, year: number, precision: number): Extremum
export function aphelion2 (planet: Planet, year: number, precision: number, cb: ExtremumCallback): void
export function aphelion2 (planet: Planet, year: number, precision: number, cb?: ExtremumCallback): Extremum | void {
  const code = planetCode(planet.name)
  return cb ? ap2(code, year, precision, planet, true, af, cb) : ap2(code, year, precision, planet, true, af)
}

function planetCode (name: string): PlanetCode {
  if (Object.prototype.hasOwnProperty.call(planetsEnum, name)) {
    return planetsEnum[name as keyof typeof planetsEnum]
  }
  throw new RangeError(`unsupported planet: ${name}`)
}

function ap2 (p: PlanetCode, y: number, d: number, v: Planet, a: boolean, f: RoundFunction): Extremum
function ap2 (p: PlanetCode, y: number, d: number, v: Planet, a: boolean, f: RoundFunction, cb: ExtremumCallback): void
function ap2 (p: PlanetCode, y: number, d: number, v: Planet, a: boolean, f: RoundFunction, cb?: ExtremumCallback): Extremum | void {
  const j1 = ap(p, y, a, f)
  if (p !== neptune) {
    return cb ? ap2a(j1, d, a, v, cb) : ap2a(j1, d, a, v)
  }

  if (cb) {
    ap2a(j1 - 5000, d, a, v, ([j0, r0]) => {
      ap2a(j1 + 5000, d, a, v, ([j2, r2]) => {
        cb((r0 > r2) === a ? [j0, r0] : [j2, r2])
      })
    })
    return
  }
  ...
  return (r0 > r2) === a ? [j0, r0] : [j2, r2]
}

function ap2a (j1Start: number, d: number, a: boolean, v: Planet): Extremum
function ap2a (j1Start: number, d: number, a: boolean, v: Planet, cb: ExtremumCallback): void
function ap2a (j1Start: number, d: number, a: boolean, v: Planet, cb?: ExtremumCallback): Extremum | void {
  let j1 = j1Start
  let j0 = j1 - d
  let j2 = j1 + d
  const rr: [number, number, number] = [
    v.position2000(j0).range,
    v.position2000(j1).range,
    v.position2000(j2).range
  ]
  ...
  while (!run()) {
    // Continue until the extremum is bracketed.
  }
  return end()
}
