 ...
 */
export function ascending (year: number) { // (year float64)  float64
  return node(year, 0)
}
 ...
 * @returns {Number} jde of the event nearest the given date.
 */
export function descending (year: number) { // (year float64)  float64
 ...
 * @private
 */
function node (y: number, h: number) { // (y, h float64)  float64
