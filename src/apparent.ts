...
import coord, { Equatorial } from './coord.ts'
import _nutation from './nutation.ts'
import precess from './precess.ts'
...
*/
export function nutation (α: number, δ: number, jd: number) : [number, number] { // (α, δ, jd float64)  (Δα1, Δδ1 float64)
 ...
 */
export function perihelion (T: number) { // (T float64)  float64
 ...
 */
export function eclipticAberration (λ: number, β: number, jd: number) : [number, number] { // (λ, β, jd float64)  (Δλ, Δβ float64)
 ...
 * coordinates of an object.
 */
export function aberration (α: number, δ: number, jd: number) : [number, number] { // (α, δ, jd float64)  (Δα2, Δδ2 float64)
  const ε = _nutation.meanObliquity(jd)
  const T = base.J2000Century(jd)
  const { lon, ano } = solar.trueLongitude(T) // eslint-disable-line no-unused-vars
  const e = solar.eccentricity(T)
 ...
 */
export function position (eqFrom: Equatorial, epochFrom: number, epochTo: number, mα: number, mδ: number) { // (eqFrom, eqTo *coord.Equatorial, epochFrom, epochTo float64, mα sexa.HourAngle, mδ sexa.Angle)  *coord.Equatorial
 ...
 */
export function aberrationRonVondrak (α: number, δ: number, jd: number) : [number, number] { // (α, δ, jd float64)  (Δα, Δδ float64)
  const T = base.J2000Century(jd)
  const r = {
    T,
  ...
  for (let i = 35; i >= 0; i--) {
    const [x, y, z] = base.at(rvTerm, i)(r)
    Xp += x
    Yp += y
    Zp += z
  }
...
const rvTerm = [
  function (r: RvCtx) : [number, number, number] { // 1
  ...
  function (r: RvCtx) : [number, number, number] { // 2
 ...
 */
export function positionRonVondrak (eqFrom: Equatorial, epochTo: number, mα: RadValue, mδ: RadValue) { // (eqFrom, eqTo *coord.Equatorial, epochTo float64, mα sexa.HourAngle, mδ sexa.Angle)  *coord.Equatorial
