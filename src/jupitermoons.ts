...
import planetelements from './planetelements.ts'
 ...
 */
export function positions (jde: number): [XYZ, XYZ, XYZ, XYZ] {
  ...
  const xy = function (u: number, r: number) {
 ...
 * @return {Array} x, y, z - coordinates of the 4 Satellites of jupiter
 */
export function e5 (jde: number, earth: Planet, jupiter: Planet, pos?: XYZ[]) {
  pos = pos ?? new Array<XYZ>(4)
  ...
  // variables assigned in following block
  let I
  const X = new Array<number>(5).fill(0)
  const Y = new Array<number>(5).fill(0)
  const Z = new Array<number>(5).fill(0)
  let R
  ...
  ;(function () {
    ...
    for (let i = 0; i < L.length; i++) {
      const [sLψ, cLψ] = base.sincos(base.at(L, i) - ψ)
      const [sB, cB] = base.sincos(base.at(B, i))
      const radius = base.at(R, i)
      X[i] = radius * cLψ * cB
      Y[i] = radius * sLψ * cB
      Z[i] = radius * sB
    }
  ...
  for (let i = 0; i < A.length; i++) {
    ...
    // step 1
    let a = base.at(X, i)
    let b = base.at(Y, i) * cI - base.at(Z, i) * sI
    let c = base.at(Y, i) * sI + base.at(Z, i) * cI
    // step 2
    a0 = a * cΦ - b * sΦ
    b = a * sΦ + b * cΦ
    a = a0
  ...
  const [sD, cD] = base.sincos(Math.atan2(base.at(A, 4), base.at(C, 4)))
  // p. 313
  for (let i = 0; i < 4; i++) {
    let x = base.at(A, i) * cD - base.at(C, i) * sD
    const y = base.at(A, i) * sD + base.at(C, i) * cD
    const z = base.at(B, i)
    // differential light time
    const d = x / base.at(R, i)
    x += Math.abs(z) / base.at(k, i) * Math.sqrt(1 - d * d)
    // perspective effect
    const W = Δ / (Δ + z / 2095)
