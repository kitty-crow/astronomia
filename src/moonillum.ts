 ...
 * @private
 */
function cosEq (α: number, δ: number, α0: number, δ0: number) {
 ...
 * @returns {number}
 */
function pa (Δ: number, R: number, cψ: number) {
 ...
 * @private
 */
function cosEcl (λ: number, β: number, λ0: number) { // (λ, β, λ0 float64)  float64
 ...
 * Result in radians.
 */
export function phaseAngle3 (jde: number) { // (jde float64)  float64
