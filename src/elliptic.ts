 ...
 */
import apparent from './apparent.ts'
import base from './base.ts'
import coord from './coord.ts'
import kepler from './kepler.ts'
import nutation from './nutation.ts'
import planetposition, { Planet } from './planetposition.ts'
import solarxyz from './solarxyz.ts'
 ...
 */
export function position (planet: Planet, earth: Planet, jde: number) { // (p, earth *pp.V87Planet, jde float64)  (α, δ float64)
  let x = 0
  let y = 0
  let z = 0
...
export class Elements {
  ...
  constructor (init: ElementsInit)
  constructor (axis: number, ecc: number, inc: number, argP: number, node: number, timeP: number)
  constructor (axisOrInit: number | ElementsInit, ecc = 0, inc = 0, argP = 0, node = 0, timeP = 0) {
    const init = typeof axisOrInit === 'number'
      ? { axis: axisOrInit, ecc, inc, argP, node, timeP }
      : axisOrInit
    this.axis = init.axis
    this.ecc = init.ecc
    this.inc = init.inc
   ...
   * all in radians.
   */
  position (jde: number, earth: Planet) { // (α, δ, ψ float64) {
    // (33.6) p. 227
    ...
    const f = (jde: number) => { // (x, y, z float64) {
 ...
 */
export function astrometricJ2000 (f: (jde: number) => { x: number; y: number; z: number }, jde: number, earth: Planet) { // (f func(float64)  (x, y, z float64), jde float64, e *pp.V87Planet) (α, δ, ψ float64)
 ...
 * Result is in Km/sec.
 */
export function velocity (a: number, r: number) { // (a, r float64)  float64
 ...
 * Result is in Km/sec.
 */
export function vAphelion (a: number, e: number) { // (a, e float64)  float64
 ...
 * Result is in Km/sec.
 */
export function vPerihelion (a: number, e: number) { // (a, e float64)  float64
  return 29.7847 * Math.sqrt((1 + e) / (1 - e) / a)
}
 ...
 */
export function length1 (a: number, e: number) { // (a, e float64)  float64
 ...
 * Result is in units used for semimajor axis, typically AU.
 */
export function length2 (a: number, e: number) { // (a, e float64)  float64
  const b = a * Math.sqrt(1 - e * e)
  const s = a + b
  const p = a * b
  const A = s * 0.5
 ...
 * Result is exact, and in units used for semimajor axis, typically AU.
 */
export function length4 (a: number, e: number) { // (a, e float64)  float64
  const b = a * Math.sqrt(1 - e * e)
  const m = (a - b) / (a + b)
  const m2 = m * m
  let sum0 = 1.0
