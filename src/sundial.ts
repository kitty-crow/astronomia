...
class Line {
  ...
  constructor (hour: number, points?: Point[]) {
 ...
 */
export function general (φ: number, D: number, a: number, z: number) { // (φ, D, a, z float64)  (lines []Line, center Point, u, ψ float64)
 ...
 */
export function equatorial (φ: number, a: number) { // (φ, a float64)  (n, s []Line)
  const tφ = Math.tan(φ)
  const n = []
  const s = []
  for (let i = 0; i < 24; i++) {
 ...
 */
export function horizontal (φ: number, a: number) { // (φ, a float64)  (lines []Line, center Point, u float64)
  const [sφ, cφ] = base.sincos(φ)
  const tφ = sφ / cφ
  const lines = []
  for (let i = 0; i < 24; i++) {
 ...
 * polar stylus.  They are in units of a, the stylus length.
 */
export function vertical (φ: number, D: number, a: number) { // (φ, D, a float64)  (lines []Line, center Point, u float64)
  const [sφ, cφ] = base.sincos(φ)
  const tφ = sφ / cφ
  const [sD, cD] = base.sincos(D)
  const lines = []
