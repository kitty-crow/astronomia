...
import deltat from '../data/deltat.ts'
...
interface MonthlyDeltaTable extends DeltaTable {
  readonly firstYM: readonly [number, number]
  readonly lastYM: readonly [number, number]
}

// avoids a cyclic dependency on the Julian calendar module
function LeapYearGregorian (y: number) {
 ...
 */
export function deltaT (dyear: number) {
  let ΔT
  if (dyear < -500) {
 ...
 * @returns {Number} ΔT in seconds.
 */
function interpolate (dyear: number, data: DeltaTable): number {
 ...
 * @returns {Number} ΔT in seconds.
 */
function interpolateData (dyear: number, data: MonthlyDeltaTable): number {
 ...
 */
function monthOfYear (dyear: number) {
  ...
  const data = MONTH_FRACTIONS[d]

  let month = 12 // TODO loop could be improved
  while (month > 0 && base.at(data, month) > f) {
    month--
  }
  const first = year + base.at(data, month)
  const last = month < 11 ? year + base.at(data, month + 2) : year + 1 + base.at(data, (month + 2) % 12)
  return { year, month, first, last }
}
const MONTH_FRACTIONS: readonly [readonly number[], readonly number[]] = [
  [ // non leap year
    0,
    0,
    0.08493150684921602,
