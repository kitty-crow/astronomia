 ...
 */
export function planetary (t1: number, t5: number, cs1: Array<Coord>, cs2: Array<Coord>) {
  if (cs1.length !== 5 || cs2.length !== 5) {
    throw new Error('Five rows required in ephemerides')
  }
  const dr: number[] = new Array(5)
  const dd: number[] = new Array(5)
  cs1.forEach((r: Coord, i: number) => {
    dr[i] = base.at(cs2, i).ra - base.at(cs1, i).ra
    dd[i] = base.at(cs2, i).dec - base.at(cs1, i).dec
  })
 ...
 *    {Number} Δd - is the amount that object 2 was "above" object 1 at the time of conjunction.
 */
export function stellar (t1: number, t5: number, c1: Coord, cs2: Array<Coord>) {
  if (cs2.length !== 5) {
    throw new Error('Five rows required in ephemerides')
  }
  const dr: number[] = new Array(5)
  const dd: number[] = new Array(5)
  cs2.forEach((r: Coord, i: number) => {
    dr[i] = base.at(cs2, i).ra - c1.ra
    dd[i] = base.at(cs2, i).dec - c1.dec
  })
  return conj(t1, t5, dr, dd)
}

const conj = function (t1: number, t5: number, dr: readonly number[], dd: readonly number[]) : [number, number] { // (t1, t5 float64, dr, dd []float64)  (t, Δd float64, err error)
