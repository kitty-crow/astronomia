...
import base, { Coord } from './base.ts' // eslint-disable-line no-unused-vars
import interp from './interpolation.ts'
export type SeparationFunction = (c1: Coord, c2: Coord) => number
 ...
 * @return {Number} angular separation between two celestial bodies
 */
export function minSep (jd1: number, jd3: number, cs1: readonly Coord[], cs2: readonly Coord[], fnSep: SeparationFunction = sep): number {
  if (cs1.length !== 3 || cs2.length !== 3) {
    throw interp.errorNot3
  }
  const y = new Array<number>(3)
  cs1.forEach((c: Coord, x: number) => {
 ...
 * @return {Number} angular separation between two celestial bodies
 */
export function minSepRect (jd1: number, jd3: number, cs1: readonly Coord[], cs2: readonly Coord[]): number {
  if (cs1.length !== 3 || cs2.length !== 3) {
    throw interp.errorNot3
  }
  const uv = function (c1: Coord, c2: Coord) : [number, number] {
    const [sind1, cosd1] = base.sincos(c1.dec)
  ...
  const us = new Array<number>(3).fill(0)
  const vs = new Array<number>(3).fill(0)
  cs1.forEach((c: Coord, x: number) => {
  ...
  const u0 = base.at(us, 0)
  const u1 = base.at(us, 1)
  const u2 = base.at(us, 2)
  const v0 = base.at(vs, 0)
  const v1 = base.at(vs, 1)
  const v2 = base.at(vs, 2)
  const up0 = (u2 - u0) / 2
  const vp0 = (v2 - v0) / 2
  const up1 = u0 + u2 - 2 * u1
  const vp1 = v0 + v2 - 2 * v1
  const up = up0
  const vp = vp0
  let dn = -(u1 * up + v1 * vp) / (up * up + vp * vp)
  let n = dn
  let u
  let v
 ...
 */
export function minSepHav (jd1: number, jd3: number, cs1: readonly Coord[], cs2: readonly Coord[]): number {
 ...
 */
export function minSepPauwels (jd1: number, jd3: number, cs1: readonly Coord[], cs2: readonly Coord[]): number {
