 ...
 */
export function sum (m1: number, m2: number): number { // (m1, m2 float64)  float64
  const x = 0.4 * (m2 - m1)
  return m2 - 2.5 * Math.log10(Math.pow(10, x) + 1)
}

/**
 * SumN returns the combined apparent magnitude of an array of stars.
 */
export function sumN (m: readonly number[]): number { // (m []float64)  float64
  let s = 0
  for (const mi of m) {
 ...
 */
export function ratio (m1: number, m2: number): number { // (m1, m2 float64)  float64
  const x = 0.4 * (m2 - m1)
 ...
 */
export function difference (ratio: number): number { // (ratio float64)  float64
 ...
 */
export function absoluteByParallax (m: number, π: number): number { // (m, π float64)  float64
 ...
 */
export function absoluteByDistance (m: number, d: number): number { // (m, d float64)  float64
