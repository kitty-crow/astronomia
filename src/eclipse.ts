...
import moonphase from './moonphase.ts'

interface EclipseGeometry {
  readonly jdeMax: number
  readonly distance: number
  readonly umbral: number
  readonly lunarAnomaly: number
}

/**
 * @private
 */
const g = function (k: number, jm: number, c1: number, c2: number): EclipseGeometry | null { // (k, jm, c1, c2 float64)  (eclipse bool, jdeMax, γ, u, Mʹ float64)
  const ck = 1 / 1236.85
  const p = Math.PI / 180
  const T = k * ck
  ...
  return { jdeMax, distance: γ, umbral: u, lunarAnomaly: Mʹ }
}
...
export type EclipseType = typeof TYPE[keyof typeof TYPE]
 ...
 */
const snap = function (y: number, q: number) { // (y, q float64)  float64
 ...
 */
export function solar (year: number) { // (year float64)  (eclipseType int, central bool, jdeMax, γ, u, p, mag float64)
  let eclipseType: EclipseType = TYPE.None
  let mag

  const geometry = g(snap(year, 0), moonphase.meanNew(year), -0.4075, 0.1721)
  if (!geometry) {
    return { type: eclipseType } // no eclipse
  }
  const { jdeMax, distance: γ, umbral: u } = geometry
  const p = u + 0.5461
  const aγ = Math.abs(γ)
  if (aγ > 1.5433 + u) {
    return { type: eclipseType } // no eclipse
 ...
 */
export function lunar (year: number) { // (year float64)  (eclipseType int, jdeMax, γ, ρ, σ, mag, sdTotal, sdPartial, sdPenumbral float64)
  let eclipseType: EclipseType = TYPE.None
  let mag
  let sdTotal
  let sdPartial
  ...
  const geometry = g(snap(year, 0.5), moonphase.meanFull(year), -0.4065, 0.1727)
  if (!geometry) {
    return { type: eclipseType } // no eclipse
  }
  const { jdeMax, distance: γ, umbral: u, lunarAnomaly: Mʹ } = geometry
  const ρ = 1.2848 + u
  const σ = 0.7403 - u
  const aγ = Math.abs(γ)
  ...
  if (eclipseType === TYPE.Total) {
    sdTotal = Math.sqrt(t * t - γ2) / n / 24
  }
  if (eclipseType === TYPE.Total || eclipseType === TYPE.Umbral) {
