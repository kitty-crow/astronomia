 ...
 * @returns {Number}
 */
export function decimalPlaces (better: Function, start: number, places: number, maxIterations: number) {
 ...
 * @returns {Number}
 */
export function fullPrecision (better: Function, start: number, maxIterations: number) {
  for (let i = 0; i < maxIterations; i++) {
    const n = better(start)
 ...
 * @returns {Number}
 */
export function binaryRoot (f: Function, lower: number, upper: number) {
  let yLower = f(lower)
  let mid = 0
  for (let j = 0; j < 52; j++) {
    mid = (lower + upper) / 2
